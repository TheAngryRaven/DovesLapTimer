...
`CourseTimerEntry _courseTimers[MAX_COURSES]` is a by-value member array — measured **28,736 B** of the manager's **30,800 B** total (host; ~17 KB on Mega with 4-byte doubles vs. its 8 KB SRAM). `pruneInactiveCourses()` only sets `active = false`; **zero bytes are ever released**. CLAUDE.md's "drops to ~5 KB after pruning" is fiction — pruning saves CPU, not memory. Note the green Mega CI cell does not contradict this: it compiles only `sector_timing_example`, and **no example anywhere instantiates `CourseManager`** — its code compiles as a TU but no instance is ever allocated. A user who declares a *global* `CourseManager` on a Mega gets a confusing avr-ld `.bss will not fit in region data` link error; a stack or heap instance compiles clean and corrupts memory at runtime. Either way the "Arduino Mega+ (minimum)" claim doesn't hold for this class, with no `static_assert`, `#warning`, or doc note saying so.
